...
const EXPERIMENT_ID = "cinematographer-actual-set-assessment";
...
async function assessPair(options: {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      const result = await assessCinematographerJourney({
        reasoning,
        journeyId: pair.journeyId,
        story: FOREST_USER_PROMPT,
      ...
      await writeJson(join(pairDir, "prompt.json"), promptContext, token);
      await writeFile(join(pairDir, "raw.txt"), result.rawText.endsWith("\n") ? result.rawText : `${result.rawText}\n`);
      await writeJson(join(pairDir, "assessment.json"), result.assessment, token);
      await writeJson(
      ...
      console.log(`${pair.journeyId} ${result.assessment.shootability} ${result.predictionId}`);
      return;
    } catch (error) {
      const completedAt = new Date().toISOString();
      lastError = error;
...
async function main(): Promise<void> {
  ...
  const manifest = {
    ...
    question:
      "Can the current Cinematographer independently distinguish actual adjacent sets that plausibly support continuous physical camera traversal from pairs that are visually related but spatially difficult or discontinuous?",

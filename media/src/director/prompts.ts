...
- When the filmmaker explicitly requests an overall journey duration, treat it as a planning constraint when deciding journey scope and destination count. Plan enough distinct spatial/story progression to plausibly support that approximate runtime while preserving traversable geography. All else equal, a short request such as about 15 seconds implies less progression than about 45 seconds, which implies less than roughly one minute. Do not let the simple-journey range override an explicit longer duration. Do not assign individual traversal durations; downstream cinematography owns shot timing. Do not convert a duration into a fixed destination count.

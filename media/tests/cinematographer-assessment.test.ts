...
test("Cinematographer assessment request asks how to shoot actual stills, not whether the video model will succeed", () => {
  ...
  assert.match(request.systemInstruction, /not predicting whether a specific video provider call will succeed/i);
  ...
  assert.match(request.systemInstruction, /same visually\/spatially consistent environment/);
  assert.match(request.systemInstruction, /must NOT force traversalConfidence lower/);
  assert.match(request.systemInstruction, /elevator interior/);
  assert.match(request.systemInstruction, /snowy mountain/);
  assert.match(request.systemInstruction, /ACTIONABLE TRAVERSAL/);
  assert.match(request.systemInstruction, /Closed is NOT inherently worse than open/);
  assert.match(request.systemInstruction, /THRESHOLD \/ GENERATIVE TRAVERSAL/);
  assert.match(request.systemInstruction, /Do not require conventional real-world architectural continuity/);
  assert.match(request.systemInstruction, /boxed in with no actionable exit/);
  assert.match(request.systemInstruction, /scene replacement rather than travel/);
  assert.match(request.systemInstruction, /Do not lower traversalConfidence merely because/);
  assert.match(request.systemInstruction, /the destination is not visible in START/);
  ...
  assert.match(request.prompt, /Do not let a low setConsistency score force traversalConfidence lower/);
  assert.match(request.prompt, /visually\/spatially consistent environment/);
  assert.match(request.prompt, /without a cut, dissolve, crossfade, teleport, or scene replacement/);

#!/usr/bin/env npx tsx
/** Hybrid + canonical-derived Camotion: inherited start′ + pristine canonical end. Not production. */
...
if (!LEG || !START || !END) {
  console.error("Usage: generate.ts <B-C|C-D|D-E> <start> <end> [durationSeconds]");
  process.exit(2);
}
    ...
    JSON.stringify(
      {
        ...
        startImage: START,
        endImage: END,
        camotionOnStart: "canonical-derived-plan-applied-to-rendered-endpoint",

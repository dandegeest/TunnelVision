#!/usr/bin/env python3
"""Read-only drop-0 validation across Kling-only projects. Experiment-local. No Pruna."""
...
def measure_seam(proj: dict, incoming: dict, outgoing: dict, role: str) -> dict:
    ...
    for label, model in (("incoming", incoming.get("model")), ("outgoing", outgoing.get("model"))):
        if not model or "kling" not in str(model).lower():
            raise RuntimeError(f"non-Kling {label} take {proj['dir']} {incoming['id']}|{outgoing['id']}: {model}")
        if "pruna" in str(model).lower():
            raise RuntimeError(f"Pruna take leaked into Kling-only run: {proj['dir']} {model}")
    L0 = compare(L, O0)
    L1 = compare(L, O1)
    O01 = compare(O0, O1)
    O12 = compare(O1, O2)

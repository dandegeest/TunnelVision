import { productionUnavailableReason } from "../project/policy";
import { useProject } from "../project/ProjectProvider";

export function ProductionBar() {
  const { project, selectedJourney, zoom, setZoom } = useProject();
  const reason = productionUnavailableReason(selectedJourney);
  ...
  return (
    <footer className="relative z-10 flex flex-none items-center justify-between gap-3 overflow-hidden border-t border-[#2a2620] bg-[#0c0b0a] px-5 py-3 text-sm">
      ...
      <p className="min-w-0 flex-1 truncate text-center text-[#9a8f7e]">{reason}</p>
